...
test("Ioctopus resolves singleton application dependencies", () => {
  ...
  const projectController = resolveApplicationDependency(
  ...
  assert.equal(typeof projectController.createProject, "function");
  assert.equal(
    projectController,
    resolveApplicationDependency(applicationInjectionTokens.projectController)
  );
});
