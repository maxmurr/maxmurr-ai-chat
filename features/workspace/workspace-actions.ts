...
export async function createFirstWorkspaceAction(name: unknown) {
  ...
  if (!parsed.success) {
    return { error: "Check workspace details and try again.", ok: false as const }
  }
  ...
  try {
    ...
    return { error: "Could not create workspace. Try again.", ok: false as const }
  }
}
...
export async function inviteFirstWorkspaceMembersAction(
  ...
  if (!parsedWorkspaceId.success || !parsedEmails.success) {
    return { error: "Check invitation details and try again.", ok: false as const }
  }

  const requestHeaders = await headers()
  const failedInvitationEmails = await inviteWorkspaceMembers(
    parsedWorkspaceId.data,
    parsedEmails.data.map((email) => ({ email, role: "member" as const })),
    (invitation) =>
      auth.api.createInvitation({
        body: invitation,
        headers: requestHeaders,
