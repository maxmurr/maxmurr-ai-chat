...
class InMemoryLibraryRepository implements LibraryRepository {
  ...
  async listOwnedFiles(folder: string | null, scope: LibraryOwnerScope) {
    return this.files
      .filter(
        (file) => file.folderId === folder && this.matches(file, scope)
      )
      .map(withoutBytes)
  }
  ...
  async moveOwnedFile(
    ...
    this.files = this.files.map((candidate) =>
      candidate === file ? { ...candidate, folderId: targetFolderId } : candidate
    )
    return true
  }
...
test("Library listings stay scoped to owner and workspace without bytes", async () => {
  ...
  assert.deepEqual(listing.files.map(({ id }) => id), [fileId])
  assert.deepEqual(listing.folders.map(({ id }) => id), [folderId])
...
test("deleting owned Folder cascades to its Files", async () => {
  ...
  assert.equal(repository.folders.some(({ id }) => id === folderId), false)
  assert.equal(repository.files.some(({ id }) => id === fileId), false)
})
...
test("moving File requires owned target Folder and accepts Library root", async () => {
  ...
  assert.equal(repository.files.find(({ id }) => id === fileId)?.folderId, folderId)

  await controller.moveFile(fileId, null, ownerScope)
  assert.equal(repository.files.find(({ id }) => id === fileId)?.folderId, null)
})
